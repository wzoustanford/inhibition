...
def train_numbers_moe_only(glu_on):
    ...
    class ClassifyModelMOE(torch.nn.Module): 
        ...
        def forward(self, x, gim_input = None): 
            ...
            x = self.moe_model(x, gim_input=gim_input, pretext_input=self.conv_base_model.pretext_input) #if gim_input is None else self.moe_model(x, gim_input)
            x = self.sm_linear(x)
    ...
    #num_steps = int(2 * len(Dtr) / batch_size)
    print(num_steps)
...
if __name__ == "__main__":
    ...
    for i, acc_list in enumerate(acc_list_all): 
        print(acc_list)
        print(f"pretext GLU, glu_on: {glu_on_list[i]}, average across 5 runs: {torch.Tensor(acc_list).float().mean()}") 
