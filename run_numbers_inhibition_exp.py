...
from models.moe_predefined_model import MoEWrapperPreDefinedRouter
...
model = MoEWrapperPreDefinedRouter(
    defined_router_model = router_conv_base_model, 
    K = 1, 
    expert_list = torch.nn.ModuleList([ClassifyModelMNIST(h_only=False).to(device) for i in range(1)]), 
    output_dim = 10, 
    glu_on = False, 
    device = device,
)
